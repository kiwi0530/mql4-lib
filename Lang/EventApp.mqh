...
#ifndef __EVENTAPP_MQH__
#define __EVENTAPP_MQH__

#include "../UI/Chart.mqh"
#include "App.mqh"
#include "Event.mqh"

#define DECLARE_EVENT_APP(_apptype)                                         \
	DECLARE_APP(_apptype)                                                   \
	void OnTimer() { dynamic_cast<EventApp*>(App::current_app).onTimer(); } \
	void OnChartEvent(const int id, const long& lparam, const double& dparam, const string& sparam) { dynamic_cast<EventApp*>(App::current_app).onChartEvent(id, lparam, dparam, sparam); }
//+------------------------------------------------------------------+
...
protected:
	void setupTimer(int seconds) {
		if (EventSetTimer(seconds)) m_hasTimer = true;
	}
	void setupMillisTimer(int millis) {
		if (EventSetMillisecondTimer(millis)) m_hasTimer = true;
	}
